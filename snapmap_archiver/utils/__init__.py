...
def match_snap_id(url: str) -> str:
    return re.search(
        r"(W7_(?:[aA-zZ0-9\-_\+]{22})(?:[aA-zZ0-9-_\+]{28})AAAAAA)", url
    ).group(1)
...
def organise_media(api_data):
    ...
    for entry in api_data:
        ...
        try:
            ...
            data_dict["location"] = entry["snapInfo"]["localitySubtitle"]["fallback"]
        try:
        ...
        try:
            ...
            try:
                data_dict["media"]["raw_url"] = entry["snapInfo"]["publicMediaInfo"][
                    "publicImage" "MediaInfo"
                ]["mediaUrl"]
                data_dict["media"]["filetype"] = "jpg"
            except KeyError:
                for i in entry["snapInfo"].items():
                    if (
                        i[0] == "streamingThumbnailInfo"
                    ):  # For some reason JSON throws an error if you just query this key directly, so you have to do it this way.
                        data_dict["media"]["raw_url"] = i[1]["infos"][-1][
                            "thumbnailUrl"
                        ]
